...
function PaneRouterProvider(props) {
  ...
  const ctx = React.useMemo(() => ({
    ...
    setView: viewId => {
      const restParams = lodash.omit(params, "view");
...
function createResolvedPaneNodeStream(_ref6) {
  ...
  const resolvedPanes$ = routerPanesStream.pipe(operators.map(rawRouterPanes => [[{
    ...
    for (let i = 0; i < curr.length; i++) {
      ...
      if (!lodash.isEqual(prevValue, currValue)) {
        return {
...
const DeskToolPane = React.memo(function DeskToolPane2(props) {
  ...
  if (!lodash.isEqual(prevParams, nextParams)) return false;
  if (!lodash.isEqual(prevPayload, nextPayload)) return false;
...
const DocumentPaneProvider = React.memo(props => {
  ...
  const toggleInspect = React.useCallback(function () {
    ...
    if (toggle) {
      ...
      paneRouter.setParams(lodash.omit(params, "inspect"));
    }
...
function TimelineMenu(_ref21) {
  ...
  const revLabel = chunk ? "".concat(lodash.upperFirst(formatTimelineEventLabel(chunk.type)), " ").concat(timeAgo) : "Current version";
...
const isMenuButton = lodash.negate(isActionButton);
...
//# sourceMappingURL=index-7ca32889.js.map
