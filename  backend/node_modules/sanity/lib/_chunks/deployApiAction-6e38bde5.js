...
var _internal = require('./_internal-ddda5304.js');
  ...
  print(output) {
    ...
    this.problemGroups.forEach(group => {
      group.problems.forEach(problem => {
        ...
        output.error("  ".concat(icon, " ").concat(lodash.upperFirst(problem.severity), ": ").concat(getPath(group.path)));
...
function getTypeName(str) {
  const name = lodash.startCase(str).replace(/\s+/g, "");
...
function extractFromSanitySchema(sanitySchema) {
  ...
  function gatherAllReferenceCandidates(type) {
    ...
    return lodash.uniqBy(allFields, "name");
  }
...
function generateTypeFilters$2(types) {
  ...
  return queryable.map(type => {
    ...
    const fields = lodash.flatten(type.fields.map(createFieldFilters$2)).filter(Boolean);
    return {
      name,
      kind: "InputObject",
...
function pluralizeTypeName(name) {
  const words = lodash.startCase(name).split(" ");
  const last = words[words.length - 1];
  const plural = pluralize__default["default"](last.toLowerCase());
  words[words.length - 1] = lodash.upperFirst(plural);
  return words.join("");
}
...
function generateTypeQueries$1(types, sortings) {
  ...
  queryable.forEach(type => {
    ...
    queries.push({
      fieldName: "all".concat(lodash.upperFirst(type.name)),
      filter: "_type == \"".concat(type.originalName || type.name, "\""),
      type: {
        kind: "List",
        isNullable: false,
...
async function deployGraphQLApiAction(args, context) {
  ...
  for (const apiDef of apiDefs) {
    ...
    try {
      ...
      const validationError = lodash.get(err, "response.body.validationError");
      spinner.fail();
...
//# sourceMappingURL=deployApiAction-6e38bde5.js.map
