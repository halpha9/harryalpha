...
function _findMatchingRoutes(node, _state) {
  ...
  const consumedParams = lodash.intersection(stateKeys, requiredParams);
  const missingParams = lodash.difference(requiredParams, consumedParams);
  const remainingParams = lodash.difference(stateKeys, consumedParams);
  ...
  const remainingState = lodash.pick(state, remainingParams);
  let matchingChild = {
    nodes: [],
    remaining: [],
    missing: []
...
function _resolvePathFromState(node, state) {
  ...
  const relative = lodash.flatten(match.nodes.map(matchNode => {
...
function useRouterState() {
  let selector = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : lodash.identity;
  const {
    state
  } = useRouter();
